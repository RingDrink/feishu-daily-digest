...
```bash
# 一次性授权所有 scope（推荐）
lark-cli auth login --scope "im:chat:readonly im:message:readonly im:message.reactions:read vc:meeting.search:read vc:meeting.meetingevent:read vc:note:read vc:record:readonly docs:doc:readonly"
```

> 建议一次授权所有 scope。多次 `auth login` 可能会替换而非合并 token，导致之前的 scope 丢失。
...
Agent 按以下顺序查找配置文件，命中第一个即用：

1. 环境变量 `FEISHU_DIGEST_CONFIG` 指定的路径
2. `~/.config/feishu-daily-digest/config.json`
3. 框架特定路径（如 `~/.openclaw/workspace-<agent>/state/daily-digest-config.json`、`~/.claude/config/daily-digest-config.json`）
4. skill 目录下的 `config.json`（仅限本地开发，**不建议**用于生产）

### Bot DM 前置条件

飞书 Bot 只能向其可用范围内的用户发 DM。刚创建的 Bot 可能需要先发布到用户可见范围，否则发送会静默失败。建议首次配置后先发一条测试消息确认 Bot 能送达。
...
1. `my_open_id` 格式合法（`ou_` 开头）且不是占位符（拒绝 `ou_xxx`）
2. `bot.app_id` 和 `bot.app_secret` 非空且不是占位符（拒绝 `***` / `cli_xxx`）
3. 发送目标 == `my_open_id`（禁止发给别人）
4. **身份验证**：用 `lark-cli auth status` 或 `lark-cli contact +get-user --as user`（不传 user_id 即查自己）获取当前授权用户的 open_id，确认与 `my_open_id` 一致。不一致则中止——防止填错 open_id 把摘要发给陌生人
5. 配置文件不在 skill 安装目录下（防止随仓库泄露）
...
```json
{"ts":"2026-06-15T08:30:01+08:00","trigger":"cron","window":"26h","chats_scanned":42,"messages_read":310,"meetings_found":2,"delivered":true,"recipient":"ou_xxx"}
```
...
3. 创建好后注册生效（具体命令请查阅 Apple 官方 launchd 文档）
...
**Q: 会议纪要读不到？**
A: 运行 `lark-cli auth login --scope "vc:meeting.search:read vc:meeting.meetingevent:read vc:note:read vc:record:readonly docs:doc:readonly"` 授权。
...
| 1.0.3 | 2026-06-15 | CC review 修复：FAQ 遗留 --as user、placeholder fail-closed、身份验证、配置查找顺序、合并 auth login、launchctl 关键词、typo |
