...
lark-cli 需要用户身份授权才能读取数据：

```bash
# 基础授权（消息读取）
lark-cli auth login --scope "im:chat:readonly im:message:readonly im:message.reactions:read" --as user

# 会议纪要授权（可选，但推荐）
lark-cli auth login --scope "vc:meeting.search:read vc:meeting.meetingevent:read vc:note:read vc:record:readonly" --as user
```

授权会生成链接/二维码，浏览器打开完成即可。所有权限均为**只读**。
...
创建 `daily-digest-config.json`（路径见下表）：
...
| `bot.app_secret` | 飞书应用 App Secret | 开放平台获取，注意安全 |

> **安全提醒**：`app_secret` 是敏感凭证。不要提交到公开仓库、不要发到群聊、不要截图分享。
...
| Claude Code | `~/.claude/skills/feishu-daily-digest/config.json` |
| Codex | `~/.codex/skills/feishu-daily-digest/config.json` |
| 通用 | skill 目录下 `config.json` |
